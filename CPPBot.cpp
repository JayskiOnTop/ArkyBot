...
#include <time.h>
#include <algorithm>
#include "src/include/discord_register.h"
#include "src/include/discord_rpc.h"
...
vector<string> listFilesInDirectory(string directoryName) {
	...
	HANDLE hFind = FindFirstFile((LPCSTR)FileName, &FindFileData);
...
void gui::Executor()
{
	...
	files = get_all_files_names_within_folder("C:\\scripts\\*.lua");
...
int main() {
	...
	Discord_Initialize(XorStr("919192705563770951").c_str(), &Handler, 1, NULL);
	DiscordRichPresence discordPresence;
	memset(&discordPresence, 0, sizeof(discordPresence));
	discordPresence.state = XorStr("Growtopia bot").c_str();
	discordPresence.details = "";
	discordPresence.startTimestamp = 0;
	discordPresence.largeImageText = "";
	discordPresence.smallImageText = XorStr("ArkyBot").c_str();
